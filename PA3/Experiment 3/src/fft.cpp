...
void transformImage(char fname[], ImageType& image, std::complex<float> transform[]) {
	...
	// call computeImage to get the image for the transform
	computeImage(fname, transform, N, M, 0);
	computeImage(fname, transform, N, M, 1);
	computeImage(fname, transform, N, M, 2);
}
